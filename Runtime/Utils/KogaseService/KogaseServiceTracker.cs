...
namespace Kogase.Utils
{
    ...
    {
        internal Action<ServiceLog, IDebugger> OnNewRequestSentEvent,
            OnNewResponseReceivedEvent,
            OnSendingWebsocketRequestEvent,
            OnReceivingWebsocketNotificationEvent;
        ...
        public void OnNewWebRequestSchedulerCreated(WebRequestScheduler newScheduler)
        {
            ...
            {
                var enhancedLoggingEnabled = logger != null ? logger.IsEnhancedLoggingEnabled() : false;
                if (enhancedLoggingEnabled && !string.IsNullOrEmpty(webRequest.RequestId))
                {
                    ...
                    if (payload != null) requestData.Payload = System.Text.Encoding.UTF8.GetString(payload);

                    var requestLog = new ServiceLog()
                    {
                        MessageId = $"{webRequest.RequestId}",
                        Timestamp = webRequest.RequestTimestamp.ToString("yyyy-MM-ddTHH:mm:ss.fffZ",
            ...
            newScheduler.PostHttpRequest += (webRequest, logger) =>
            {
                ...
                if (enhancedLoggingEnabled && !string.IsNullOrEmpty(webRequest.RequestId))
                {
                    ...
                    var responseContentType = webRequest.GetResponseHeader(KogaseWebRequest.ResponseContentTypeHeader);
                    if (!string.IsNullOrEmpty(responseContentType)) requestData.ContentType = responseContentType;

                    var requestLog = new ServiceLog()
                    {
                        MessageId = $"{webRequest.RequestId}",
                        Timestamp = webRequest.ResponseTimestamp.ToString("yyyy-MM-ddTHH:mm:ss.fffZ",
                            System.Globalization.CultureInfo.InvariantCulture),
                        Type = (int)DataType.REQUEST,
