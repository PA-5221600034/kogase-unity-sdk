...
namespace Kogase.Core
{
    internal class WebRequestScheduler
    {
        ...
        WebRequestTaskOrderComparer orderComparer = new();
        ...
        internal async void ExecuteWebTask(WebRequestTask task)
        {
            ...
            if (task.DelayMs > 0)
            {
                ...
                while (!isDone) await System.Threading.Tasks.Task.Yield();
            }
            ...
            using (var webRequest = task.CreateWebRequest())
            {
                using (var webRequestCancelTokenSource = new System.Threading.CancellationTokenSource())
                {
                    ...
                    var isTimeout = false;
                    heartBeat.Wait(new WaitTimeCommand(timeoutSeconds,
                        cancellationToken: webRequestCancelTokenSource.Token, onDone: () =>
                        {
                    ...
                    while (!asyncOp.isDone && !isTimeout) await System.Threading.Tasks.Task.Yield();
