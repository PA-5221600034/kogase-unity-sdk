...
namespace Kogase.Core
{
    ...
    public class HttpClient
    {
        ...
        public void SetImplicitPathParams(IDictionary<string, string> pathParams)
        {
            ...
            {
                pathParamsDict[param.Key] = param.Value;
            }
        }
        ...
        public async Task<HttpSendResult> SendRequestAsync(IHttpRequest requestInput)
        {
            ...
            var rand = new Random();
            uint retryDelayMs = initialDelayMs;
            var stopwatch = new Stopwatch();
            stopwatch.Start();

            IHttpRequest request = requestInput;
            
            ApplyImplicitAuthorization(request, out Error applyAuthErr);
            ...
            ApplyAdditionalData(request, additionalHeaderInfoDict, out Error applyAdditionalDataErr);
            ...
            {
                return new HttpSendResult(null, new Error(ErrorCode.INVALID_REQUEST, "Invalid uri format: ", request.Url));
            }
            ...
            do
            {
                ...
                int timeoutMs = (int)(totalTimeoutMs - stopwatch.ElapsedMilliseconds);
                ...
                void OnSendRequestComplete(HttpSendResult result) => sendResult = result;
        ...
        public IEnumerator SendRequest(IHttpRequest requestInput, Action<IHttpResponse, Error> callback)
        {
            ...
            do
            {
                if (state == RequestState.PAUSED)
                {
                    ...
                    {
                        request.Headers.Remove("Authorization");
                        ApplyImplicitAuthorization(request, out Error applyError);
        ...
        bool TryResolveUri(IHttpRequest request)
        {
            ...
            if (!Uri.TryCreate(baseUri, request.Url, out Uri uri)) return false;
        ...
        void ApplyImplicitAuthorization(IHttpRequest request, out Error err)
        {
            ...
            switch (request.AuthType)
            {
                case HttpAuthType.API_KEY:
                    ...
                    {
                        errorResult = new Error(ErrorCode.INVALID_REQUEST, "failed to apply auth code from url :", request.Url);
                        err = errorResult;
                        break;
                    }
                ...
                case HttpAuthType.BEARER:
                    ...
                    {
                        errorResult = new Error(ErrorCode.IS_NOT_LOGGED_IN, "failed to apply auth code from url :", request.Url);
                        err = errorResult;
                        break;
                    }
        ...
        bool CheckRequireRetry(IHttpRequest httpRequest, IHttpResponse httpResponse, Error error, Action<IHttpRequest> onNetworkError, Action<IHttpRequest> onServerError)
        {
